...
JointStory script, based on version "JointStory_harvard_3.py",
originally rewritten for the Harvard - Dartmouth
connection.

Intended for python3.9

For argument options, type
$ python hyperscan_main.py --help
...
multiple OS (so far only tested on Ubuntu 20.04 though)

- Current version handles TTLs in a simple way: it
records and timestamps keyboard inputs that equal a preset value -
this method should supports scanner sites at Darmtouth and Harvard.
The earlier command line option --LOGTTL that controlled TTL logging 
via a serial port reading subprocess does not exist any more.
...
- Does not include wrapper around stunclient any more.

- Uses PsychoPy under python3.9! Finally!
...
def magicNumbers(ROLE, TRIALTYPE, WHICHSEED):
    ...
    return(punchTimeout, CHANNELS, RATE, FORMAT, savefileOut,
           ...
           PortIn, PortOut, PortComm, turntime, timeDiff,
...
def EscCleanup(queueInput, queueOutput, audioInput, audioOutput,
               fLog, fTTL, win):
...
# def networkInit(NAT, portIn, portOut):
def networkInit(NAT, portIn, portOut, punchTimeout):
...
def goGo(NAT, ROLE, TRIALTYPE, WHICHSEED):
    ...
    [punchTimeout, CHANNELS, RATE, FORMAT, savefileOut,
     savefileIn1, savefileIn2, savefileLog, savefileTTL, portIn, portOut,
     PortIn, PortOut, PortComm, turntime, timeDiff, instText1,
    ...
    # networkInit
    networkInit(NAT, portIn, portOut, punchTimeout)
    ...
    while True:
        ...
        if keys:
            ...
            # escape quits
                ...
                EscCleanup(queueInput, queueOutput, audioInput,
                           audioOutput, fLog, fTTL, win)
                return
            ...
            while True:
                ...
                if keys:
                    ...
                    # escape quits
                        ...
                        EscCleanup(queueInput, queueOutput,
