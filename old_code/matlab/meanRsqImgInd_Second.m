function meanRsqImgInd_Second(permut)
...
% save path
saveFile = [folder1, 'permResultsInd.mat'];
...
realN = size(RsqAllInd,1);
pseudoN = size(RsqIndPseudo,1);

% test statistic is mean Rsq
realDiff = mean(RsqAllInd, 1)-mean(RsqIndPseudo,1);

% overall data
data = [RsqAllInd;RsqIndPseudo];
...
for v = 1:voxelN
    ...
    pInd(v) = (sum(diffNull(:,v) >= realDiff(v)) / permut);
    
end
    

save(saveFile, 'RsqImgInd', 'pInd');
