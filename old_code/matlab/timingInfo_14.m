function timingInfo_14(pairN, runN)
...
%
% !!! Only pair 14 !!!
% there is repeated 1st run there so needs special treatment :)
%
...
if ~ismember(pairN,14)
    error('Input arg pairN should be 14, this script is only for that pair');
end
if ~ismember(runN,[1,2,5])
    error('Input arg runN should be 1 or 2 or 5'); % 5 is repeated run1
end
...
elseif runN == 5
    saveFile = [folderParts{1},'timingInfo_', num2str(pairN), '_repeated1.mat'];
end
