...
parseEPI_First_v2(dbicFile, dhmcFile, timingInfo, turnInterpTRs)
...
counter=0;
for turnStart = speechTurnStamps.dhmc(1:turnNo)'
    counter=counter+1;
...
dhmcDataTurns = interp1(ttls.dhmc, dhmcData, turnTimeStamps, interpMethod);
...
if ~isequal(size(dhmcDataTurns), [length(turnInterpTRs), turnNo, voxelN])
    disp([char(10), 'Stg is wrong, dhmcDataTurns has the wrong size: ']);
    disp(size(dhmcDataTurns));
end
...
for currentTurn = 1:turnNo
    ...
    if mod(currentTurn, 2) == 1
        dhmcSpeaker((ceil(currentTurn/2)-1)*length(turnInterpTRs)+1:ceil(currentTurn/2)*length(turnInterpTRs), :) = squeeze(dhmcDataTurns(:,currentTurn,:));
    ...
    elseif mod(currentTurn, 2) == 0
        dhmcListener((currentTurn/2-1)*length(turnInterpTRs)+1:(currentTurn/2)*length(turnInterpTRs), :) = squeeze(dhmcDataTurns(:,currentTurn,:));
